...
for (l1_off = mmu_info->vstart; l1_off <= mmu_info->vend; l1_off++)
{
    ...
    if (*mmu_l1 & ARCH_MMU_USED_MASK)//判断一级页表是否有映射（如果没做映射）
    {
        mmu_l2 = (size_t *)((*mmu_l1 & ~ARCH_PAGE_TBL_MASK) - mmu_info->pv_off);//把物理地址转换成虚拟地址
        for (l2_off = 0; l2_off < (ARCH_SECTION_SIZE/ARCH_PAGE_SIZE); l2_off++)
        {
            if (*(mmu_l2 + l2_off) & ARCH_MMU_USED_MASK)//不断的找没人使用的内存，直到找到没人用的一片内存
            {
                /* in use */
                n = 0;
            }
